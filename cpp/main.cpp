/**
 * main.cpp — Monte Carlo Minesweeper solver demo & benchmark.
 *
 * Usage:
 *   ./minesweeper_mc [--rows R] [--cols C] [--mines M]
 *                    [--samples S] [--games G] [--quiet]
 *
 * Examples:
 *   ./minesweeper_mc                          # single verbose beginner game
 *   ./minesweeper_mc --games 100 --quiet      # benchmark, 100 games
 *   ./minesweeper_mc --rows 16 --cols 30 --mines 99 --samples 1000
 */
#include "game.hpp"
#include "solver.hpp"
...
static bool run_game(int rows, int cols, int mines, int n_samples,
    ...
    MonteCarloSolver solver(n_samples);

    // First move: centre cell (no information yet, any choice is equivalent)
    ...
    while (!game.game_over) {
        auto probs = solver.get_probabilities(game.board, rows, cols, mines);
        int move = solver.best_move(game.board, rows, cols, mines);
        if (move < 0) break;
...
static void benchmark(int rows, int cols, int mines, int n_samples,
    ...
    std::cout << "\n\nBenchmark (" << rows << "x" << cols
              << ", " << mines << " mines, " << n_samples << " samples/game)\n";
