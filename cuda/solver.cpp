...
MonteCarloSolver::get_probabilities(const std::vector<int>& board,
    ...
    // -----------------------------------------------------------------------
    // Sample loop — OpenMP parallel version
    // -----------------------------------------------------------------------
    #pragma omp parallel
    {
        std::mt19937 local_rng(_rng() ^ ((unsigned)omp_get_thread_num() * 2654435761u));
        std::vector<double> local_hits(N, 0.0);
        int local_valid = 0;

        #pragma omp for schedule(dynamic)
        for (int s = 0; s < n_samples; ++s) {
            auto b_result = _sample_boundary(boundary, constraints, min_b, max_b, local_rng);
            ...
            std::shuffle(interior_copy.begin(), interior_copy.end(), local_rng);

            ++local_valid;
            for (int j = 0; j < (int)boundary.size(); ++j)
                local_hits[boundary[j]] += (*b_result)[j];
            for (int j = 0; j < i_mines; ++j)
                local_hits[interior_copy[j]] += 1.0;
        }
